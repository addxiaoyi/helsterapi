...
func Distribute() func(c *gin.Context) {
	return func(c *gin.Context) {
		...
		if ok {
			channelIDText, validType := channelId.(string)
			if !validType || strings.TrimSpace(channelIDText) == "" {
				abortWithOpenAiMessage(c, http.StatusBadRequest, i18n.T(c, i18n.MsgDistributorInvalidChannelId))
				return
			}
			id, err := strconv.Atoi(channelIDText)
			if err != nil {
				abortWithOpenAiMessage(c, http.StatusBadRequest, i18n.T(c, i18n.MsgDistributorInvalidChannelId))
				return
			}
			...
			if shouldSelectChannel {
				...
				// Image providers are kept in the GPT billing group. Preserve an
				// explicit token/playground group, but make the default user route
				// image requests to GPT when that group is available to the user.
				if common.IsImageGenerationModel(modelRequest.Model) && usingGroup == "default" {
					userGroup := common.GetContextKeyString(c, constant.ContextKeyUserGroup)
					if service.GroupInUserUsableGroups(userGroup, "gpt") {
						usingGroup = "gpt"
