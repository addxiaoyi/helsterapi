...
func authHelper(c *gin.Context, minRole int) {
	...
	// SECURITY FIX: 完全移除对客户端提供的 New-Api-User header 的依赖
	// 只信任服务端 session 中的用户信息，防止权限提升攻击
	idValue, idOK := id.(int)
	statusValue, statusOK := status.(int)
	roleValue, roleOK := role.(int)
	usernameValue, usernameOK := username.(string)

	if !idOK || idValue <= 0 || !statusOK || !roleOK || !usernameOK {
		c.JSON(http.StatusUnauthorized, gin.H{
			"success": false,
			"message": common.TranslateMessage(c, i18n.MsgAuthNotLoggedIn),
		})
	...
	if !useAccessToken {
		if cookieValue, cookieErr := c.Cookie("session_v2"); cookieErr == nil {
			revoked, sessionErr := model.IsSessionRevoked(cookieValue)
			if sessionErr != nil {
				common.SysLog("session revocation check failed: " + sessionErr.Error())
			} else if revoked {
				c.JSON(http.StatusUnauthorized, gin.H{"success": false, "message": "session revoked or expired"})
				c.Abort()
				return
			}
			if _, sessionErr := model.RegisterUserSession(idValue, cookieValue, c.GetHeader("User-Agent"), c.ClientIP()); sessionErr != nil {
				common.SysLog("session registration failed: " + sessionErr.Error())
			}
		}
	}
