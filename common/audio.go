...
func getAACDuration(r io.ReadSeeker) (float64, error) {
	...
	maxBytes := constant.MaxFileDownloadMB * 1024 * 1024
	if maxBytes <= 0 {
		maxBytes = 64 * 1024 * 1024
	}
	// AAC parsing needs the complete byte slice, so keep it within the upload limit.
	data, err := io.ReadAll(io.LimitReader(r, int64(maxBytes)+1))
	if err != nil {
		return 0, errors.Wrap(err, "failed to read aac file")
	}
	if len(data) > maxBytes {
		return 0, fmt.Errorf("aac file exceeds maximum size of %d MB", maxBytes/(1024*1024))
	}
