...
// ApiErrorI18nCode is like ApiErrorI18n but adds a stable machine-readable
// `code` field so the frontend can map the error to its i18n dictionary
// without depending on the (translated) message string.
func ApiErrorI18nCode(c *gin.Context, code, key string, args ...map[string]any) {
	msg := TranslateMessage(c, key, args...)
	c.JSON(http.StatusOK, gin.H{
		"success": false,
		"message": msg,
