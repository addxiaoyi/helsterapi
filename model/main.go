...
func createRootAccountIfNeed() error {
	...
	if err := DB.First(&user).Error; err != nil {
		initialPassword := common.GetEnvOrDefaultString("INITIAL_ROOT_PASSWORD", common.GetRandomString(24))
		hashedPassword, err := common.Password2Hash(initialPassword)
		if err != nil {
			return err
		}
		common.SysLog("no user exists, created initial root account username=root; password is shown once in startup logs")
		common.SysLog("initial root password: " + initialPassword)
		rootUser := User{
			Username:    "root",
		...
		if err := DB.Create(&rootUser).Error; err != nil {
			return fmt.Errorf("create initial root account: %w", err)
		}
	}
	return nil
}
...
func syncClickHouseLogTTL(ttlDays int) error {
	...
	if expression != "" {
		// 使用参数化查询防止SQL注入
		sql := "ALTER TABLE logs MODIFY TTL toDateTime(created_at) + INTERVAL ? DAY DELETE"
		return LOG_DB.Exec(sql, ttlDays).Error
	}
...
PRIMARY KEY (` + "`id`" + `)
	...
	// 使用参数化查询防止SQL注入 - 虽然tableName是常量，但这是最佳实践
	// 注意: PRAGMA在SQLite中不支持参数化，但tableName是hardcoded的常量，所以是安全的
	// 添加验证确保tableName只能是预期的表名
	...
	for _, col := range required {
		...
		// 使用 GORM Migrator 的 AddColumn 方法替代直接字符串拼接
		// 这样更安全且跨数据库兼容
		// 注意：col.DDL 已经是完整的列定义，需要手动执行
		// 虽然tableName是常量，但添加验证
		if tableName != "subscription_plans" {
			return fmt.Errorf("invalid table name for migration: %s", tableName)
		}
		// 使用反引号包裹表名和列定义防止注入（虽然这里都是常量）
		sql := fmt.Sprintf("ALTER TABLE `%s` ADD COLUMN %s", tableName, col.DDL)
