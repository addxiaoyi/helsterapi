...
func (user *User) ValidateAndFill() (err error) {
	...
	username := normalizeLoginIdentifier(user.Username)
	...
	// 调试日志
	common.SysLog(fmt.Sprintf("Login attempt for user %s: password_match=%v, status=%d, status_enabled=%d",
		username, okay, user.Status, common.UserStatusEnabled))
...
func ValidateAccessToken(token string) (*User, error) {
	...
	// 检查 AccessToken 是否过期
	expired, err := IsAccessTokenExpired(user.Id)
	if err != nil {
		return nil, fmt.Errorf("%w: failed to check token expiry: %v", ErrDatabase, err)
	}
	if expired {
		// Token 已过期，返回 nil（未授权）
		return nil, nil
	}
...
func increaseUserQuota(id int, quota int) (err error) {
	update := DB.Model(&User{}).Where("id = ?", id).Update("quota", gorm.Expr("quota + ?", quota))
	if update.Error != nil {
		return update.Error
	}
...
func updateUserUsedQuotaAndRequestCount(id int, quota int, count int) {
	update := DB.Model(&User{}).Where("id = ?", id).Updates(
	...
	if update.Error != nil {
		common.SysLog("failed to update user used quota and request count: " + update.Error.Error())
	} else if update.RowsAffected == 0 {
		common.SysLog(fmt.Sprintf("user quota settlement affected no rows: user_id=%d", id))
		return
	}
...
func updateUserQuotaUsedQuotaAndRequestCount(id int, quota int, usedQuota int, requestCount int) {
	...
	if update.Error != nil {
		common.SysLog("failed to batch update user quota, used quota and request count: " + update.Error.Error())
	} else if update.RowsAffected == 0 {
		common.SysLog(fmt.Sprintf("batch user quota settlement affected no rows: user_id=%d", id))
	}
}
...
func updateUserUsedQuota(id int, quota int) {
	...
	if update.Error != nil {
		common.SysLog("failed to update user used quota: " + update.Error.Error())
	} else if update.RowsAffected == 0 {
		common.SysLog(fmt.Sprintf("user used quota update affected no rows: user_id=%d", id))
	}
}

func updateUserRequestCount(id int, count int) {
	update := DB.Model(&User{}).Where("id = ?", id).Update("request_count", gorm.Expr("request_count + ?", count))
	if update.Error != nil {
		common.SysLog("failed to update user request count: " + update.Error.Error())
	} else if update.RowsAffected == 0 {
		common.SysLog(fmt.Sprintf("user request count update affected no rows: user_id=%d", id))
	}
}
