...
func addUserQuota(tx *gorm.DB, userID int, quota int64) error {
	update := tx.Model(&User{}).Where("id = ?", userID).Update("quota", gorm.Expr("quota + ?", quota))
	if update.Error != nil {
		return update.Error
	}
...
func CompleteEpayTopUp(tradeNo, actualPaymentMethod, callerIP string) error {
	...
	err := DB.Transaction(func(tx *gorm.DB) error {
		...
		if err := addUserQuota(tx, topUp.UserId, int64(quotaToAdd)); err != nil {
			return err
		}
		completed = &topUp
		return nil
...
func Recharge(referenceId string, customerId string, callerIp string) (err error) {
	...
	err = DB.Transaction(func(tx *gorm.DB) error {
		...
		userUpdate := tx.Model(&User{}).Where("id = ?", topUp.UserId).Updates(map[string]interface{}{"stripe_customer": customerId, "quota": gorm.Expr("quota + ?", quota)})
