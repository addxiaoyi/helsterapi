...
func GetRandomSatisfiedChannel(group string, model string, retry int, requestPath string) (*Channel, error) {
	...
	// Keep weighted distribution while preferring known lower latency when
	// candidates otherwise have the same routing priority.
	sortChannelsForRouting(targetChannels)
...
func sortChannelsForRouting(channels []*Channel) {
	sort.SliceStable(channels, func(i, j int) bool {
		left, right := channels[i], channels[j]
		if left.ResponseTime == 0 && right.ResponseTime != 0 {
			return false
		}
		if right.ResponseTime == 0 && left.ResponseTime != 0 {
			return true
		}
