...
func GetTwoFAStats() (map[string]interface{}, error) {
	...
	var twoFAs []TwoFA
	if err := DB.Select("user_id, is_enabled, locked_until, last_used_at").Find(&twoFAs).Error; err != nil {
		return nil, err
	}
	twoFAByUser := make(map[int]TwoFA, len(twoFAs))
	for _, twoFA := range twoFAs {
		twoFAByUser[twoFA.UserId] = twoFA
	}
	userDetails := make([]map[string]interface{}, 0, len(users))
	var lockedUsers int
	for _, user := range users {
		twoFA, exists := twoFAByUser[user.Id]
		locked := exists && twoFA.IsLocked()
		if locked {
			lockedUsers++
		}
		userDetails = append(userDetails, map[string]interface{}{
			...
			"email":              user.Email,
			"two_factor_enabled": exists && twoFA.IsEnabled,
			"locked":             locked,
