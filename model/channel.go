...
func updateChannelUsedQuota(id int, quota int) {
	update := DB.Model(&Channel{}).Where("id = ?", id).Update("used_quota", gorm.Expr("used_quota + ?", quota))
	if update.Error != nil {
		common.SysLog(fmt.Sprintf("failed to update channel used quota: channel_id=%d, delta_quota=%d, error=%v", id, quota, update.Error))
	} else if update.RowsAffected == 0 {
		common.SysLog(fmt.Sprintf("channel used quota update affected no rows: channel_id=%d, delta_quota=%d", id, quota))
	}
}
