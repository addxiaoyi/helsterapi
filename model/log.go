...
func GetUserErrorStats(username string, startTimestamp, endTimestamp int64, modelName, tokenName string, channel int, group string) (ErrorStats, error) {
	query := LOG_DB.Model(&Log{}).Select(
		"COALESCE(SUM(CASE WHEN type = ? THEN 1 ELSE 0 END), 0) requests, COALESCE(SUM(CASE WHEN type = ? THEN 1 ELSE 0 END), 0) errors",
	...
	if err := query.Scan(&stats).Error; err != nil {
		return ErrorStats{}, errors.New("查询错误率统计失败")
	}
	if stats.Requests+stats.Errors > 0 {
		stats.Rate = float64(stats.Errors) / float64(stats.Requests+stats.Errors)
	}
	return stats, nil
}
