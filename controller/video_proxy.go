...
const (
	privateVideoCacheControl = "private, no-store"
	maxVideoProxyBytes       = 512 << 20
)
...
func VideoProxy(c *gin.Context) {
	...
	for _, key := range []string{"Content-Type", "Accept-Ranges", "ETag", "Last-Modified"} {
	...
	if resp.ContentLength > maxVideoProxyBytes {
		videoProxyError(c, http.StatusBadGateway, "server_error", "Video content exceeds maximum size")
		return
	}
	c.Writer.WriteHeader(resp.StatusCode)
	written, copyErr := io.Copy(c.Writer, io.LimitReader(resp.Body, maxVideoProxyBytes+1))
	if copyErr != nil {
		logger.LogError(c.Request.Context(), fmt.Sprintf("Failed to stream video content: %s", copyErr.Error()))
	} else if written > maxVideoProxyBytes {
		logger.LogError(c.Request.Context(), "Video content exceeded maximum size")
	}
}
...
func writeVideoDataURL(c *gin.Context, dataURL string) error {
	...
	if len(payload) > ((maxVideoProxyBytes+2)/3)*4 {
		return fmt.Errorf("video data exceeds maximum size")
	}
