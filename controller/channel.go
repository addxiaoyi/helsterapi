...
const maxChannelModelResponseBytes = 4 << 20
...
func FetchModels(c *gin.Context) {
	...
	if err := json.NewDecoder(io.LimitReader(response.Body, maxChannelModelResponseBytes)).Decode(&result); err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{
			"success": false,
			"message": err.Error(),
		})
