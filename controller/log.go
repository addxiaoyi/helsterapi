...
func GetLogsSelfErrorStats(c *gin.Context) {
	stats, err := model.GetUserErrorStats(
		c.GetString("username"),
		parseInt64Query(c, "start_timestamp"),
		parseInt64Query(c, "end_timestamp"),
		c.Query("model_name"), c.Query("token_name"), parseIntQuery(c, "channel"), c.Query("group"),
	)
	if err != nil {
		common.ApiError(c, err)
		return
