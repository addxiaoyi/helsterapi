...
const (
	...
	maxSyncTimeoutSeconds       = 5 * 60
	defaultEndpoint             = "/api/pricing"
	maxSyncUpstreams            = 100
...
func FetchUpstreamRatios(c *gin.Context) {
	...
	if req.Timeout > maxSyncTimeoutSeconds {
		req.Timeout = maxSyncTimeoutSeconds
	}
	if len(req.Upstreams) > maxSyncUpstreams || len(req.ChannelIDs) > maxSyncUpstreams {
		c.JSON(http.StatusBadRequest, gin.H{"success": false, "message": "上游数量不能超过 100"})
		return
	}
