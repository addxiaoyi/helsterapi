...
func Login(c *gin.Context) {
	...
	if err != nil {
		...
		case errors.Is(err, model.ErrDatabase):
			...
			common.ApiErrorI18nCode(c, "login_db_error", i18n.MsgDatabaseError)
		case errors.Is(err, model.ErrUserEmptyCredentials):
			common.ApiErrorI18nCode(c, "login_invalid_params", i18n.MsgInvalidParams)
		default:
			common.ApiErrorI18nCode(c, "invalid_credentials", i18n.MsgUserUsernameOrPasswordError)
		}
		return
	}
...
func setupLoginSession(user *model.User, c *gin.Context) bool {
	...
	csrfToken, err := middleware.GenerateCSRFToken(session)
	if err != nil {
		common.SysLog(fmt.Sprintf("Failed to generate CSRF token for user %d: %v", user.Id, err))
		common.ApiErrorI18n(c, i18n.MsgUserSessionSaveFailed)
		return false
	}
...
func GenerateAccessToken(c *gin.Context) {
	...
	// 设置 AccessToken 过期时间（90天）
	if err := model.SetAccessTokenExpiry(user.Id, 90); err != nil {
		common.SysLog("failed to set access token expiry: " + err.Error())
		// 不阻止 token 生成，只记录错误
	}
...
func TopUp(c *gin.Context) {
	...
	if err != nil {
		...
		keyFingerprint := sha256.Sum256([]byte(req.Key))
		logger.LogError(c, fmt.Sprintf("failed to redeem key fingerprint=%x for user %d: %s", keyFingerprint[:8], id, err.Error()))
		return
	}
	c.JSON(http.StatusOK, gin.H{
		"success": true,
...
func UpdateUserSetting(c *gin.Context) {
	...
	if req.QuotaWarningType == dto.NotifyTypeBark {
		...
		// 验证URL格式
		barkURL, err := url.Parse(req.BarkUrl)
		if err != nil || barkURL.Host == "" {
			common.ApiErrorI18n(c, i18n.MsgSettingBarkUrlInvalid)
			return
		}
		// 检查是否是HTTP或HTTPS
		if barkURL.Scheme != "https" && barkURL.Scheme != "http" {
	...
	if req.QuotaWarningType == dto.NotifyTypeGotify {
		...
		// 验证URL格式
		gotifyURL, err := url.Parse(req.GotifyUrl)
		if err != nil || gotifyURL.Host == "" {
			common.ApiErrorI18n(c, i18n.MsgSettingGotifyUrlInvalid)
			return
		}
		// 检查是否是HTTP或HTTPS
		if gotifyURL.Scheme != "https" && gotifyURL.Scheme != "http" {
			common.ApiErrorI18n(c, i18n.MsgSettingUrlMustHttp)
			return
		}
	}
