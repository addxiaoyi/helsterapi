...
func syncHTTPTimeoutSeconds(defaultValue int) int {
	seconds := common.GetEnvOrDefault("SYNC_HTTP_TIMEOUT_SECONDS", defaultValue)
	if seconds < 1 {
		return defaultValue
	}
	if seconds > 5*60 {
...
func newHTTPClient() *http.Client {
	timeoutSec := syncHTTPTimeoutSeconds(10)
...
func SyncUpstreamModels(c *gin.Context) {
	...
	if len(req.Overwrite) > 100 {
		c.JSON(http.StatusBadRequest, gin.H{"success": false, "message": "overwrite 条目不能超过 100 个"})
		return
	}
	for _, item := range req.Overwrite {
		if len(item.Fields) > 50 {
			c.JSON(http.StatusBadRequest, gin.H{"success": false, "message": "单个模型字段不能超过 50 个"})
			return
		}
	}
	// 1) 获取未配置模型列表
	...
	timeoutSec := syncHTTPTimeoutSeconds(15)
