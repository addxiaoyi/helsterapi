...
func Relay(c *gin.Context, relayFormat types.RelayFormat) {
	...
	for ; retryParam.GetRetry() <= common.RetryTimes; retryParam.IncreaseRetry() {
		...
		if newAPIError == nil {
			service.RecordChannelHealthSuccess(channel.Id)
			relayInfo.LastError = nil
			return
		}
...
func processChannelError(c *gin.Context, channelError types.ChannelError, err *types.NewAPIError) {
	healthThresholdReached := service.RecordChannelHealthFailure(channelError.ChannelId)
	...
	if channelError.AutoBan && (service.ShouldDisableChannel(err) || healthThresholdReached) {
		gopool.Go(func() {
