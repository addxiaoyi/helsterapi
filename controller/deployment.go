...
func GetDeploymentLogs(c *gin.Context) {
	...
	if startTime := c.Query("start_time"); startTime != "" {
		...
		if err != nil {
			common.ApiErrorMsg(c, "start_time must be RFC3339")
			return
		}
		opts.StartTime = &t
	}
	if endTime := c.Query("end_time"); endTime != "" {
		...
		if err != nil {
			common.ApiErrorMsg(c, "end_time must be RFC3339")
			return
		}
		opts.EndTime = &t
	}
	if opts.StartTime != nil && opts.EndTime != nil && opts.EndTime.Before(*opts.StartTime) {
		common.ApiErrorMsg(c, "end_time must not be earlier than start_time")
		return
	}
