...
const (
	...
	channelUpstreamModelUpdateMaxModelsPerRequest         = 1000
...
func ApplyChannelUpstreamModelUpdates(c *gin.Context) {
	...
	if len(req.AddModels) > channelUpstreamModelUpdateMaxModelsPerRequest ||
		len(req.IgnoreModels) > channelUpstreamModelUpdateMaxModelsPerRequest ||
		len(req.RemoveModels) > channelUpstreamModelUpdateMaxModelsPerRequest {
		common.ApiError(c, fmt.Errorf("每类模型列表最多允许 %d 项", channelUpstreamModelUpdateMaxModelsPerRequest))
		return
	}
