...
func RequestCreemPay(c *gin.Context) {
	...
	bodyBytes, err := io.ReadAll(http.MaxBytesReader(c.Writer, c.Request.Body, maxPaymentRequestBodyBytes))
	if err != nil {
...
func CreemWebhook(c *gin.Context) {
	...
	logger.LogInfo(c.Request.Context(), fmt.Sprintf("Creem webhook 收到请求 path=%q client_ip=%s body_bytes=%d", c.Request.URL.Path, c.ClientIP(), len(bodyBytes)))
	if signature == "" {
		logger.LogWarn(c.Request.Context(), fmt.Sprintf("Creem webhook 缺少签名 path=%q client_ip=%s body_bytes=%d", c.Request.URL.Path, c.ClientIP(), len(bodyBytes)))
	...
	if !verifyCreemSignature(string(bodyBytes), signature, setting.CreemWebhookSecret) {
		logger.LogWarn(c.Request.Context(), fmt.Sprintf("Creem webhook 验签失败 path=%q client_ip=%s body_bytes=%d", c.Request.URL.Path, c.ClientIP(), len(bodyBytes)))
		c.AbortWithStatus(http.StatusUnauthorized)
		return
	}
	...
	if err := c.ShouldBindJSON(&webhookEvent); err != nil {
		logger.LogError(c.Request.Context(), fmt.Sprintf("Creem webhook 解析失败 path=%q client_ip=%s error=%q body_bytes=%d", c.Request.URL.Path, c.ClientIP(), err.Error(), len(bodyBytes)))
		c.AbortWithStatus(http.StatusBadRequest)
		return
	}
...
func genCreemLink(ctx context.Context, referenceId string, product *CreemProduct, email string, username string) (string, error) {
	...
	// 读取响应
	body, err := io.ReadAll(io.LimitReader(resp.Body, maxPaymentRequestBodyBytes+1))
	if err != nil {
		return "", fmt.Errorf("读取响应失败: %v", err)
	}
	if int64(len(body)) > maxPaymentRequestBodyBytes {
		return "", fmt.Errorf("支付服务响应超过 %d 字节", maxPaymentRequestBodyBytes)
	}
