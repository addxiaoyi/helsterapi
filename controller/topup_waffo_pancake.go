...
func WaffoPancakeWebhook(c *gin.Context) {
	...
	if err != nil {
		logger.LogWarn(c.Request.Context(), fmt.Sprintf("Waffo Pancake webhook 验签失败 path=%q client_ip=%s body_bytes=%d error=%q", c.Request.URL.Path, c.ClientIP(), len(bodyBytes), err.Error()))
