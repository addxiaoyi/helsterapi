...
const maxVideoTaskResponseBytes int64 = 1 << 20

func readVideoTaskResponse(body io.Reader) ([]byte, error) {
	response, err := io.ReadAll(io.LimitReader(body, maxVideoTaskResponseBytes+1))
	if err != nil {
		return nil, err
	}
	if int64(len(response)) > maxVideoTaskResponseBytes {
		return nil, fmt.Errorf("task response exceeds %d bytes", maxVideoTaskResponseBytes)
	}
	return response, nil
}
...
func getGeminiVideoURL(channel *model.Channel, task *model.Task, apiKey string) (string, error) {
	...
	body, err := readVideoTaskResponse(resp.Body)
	if err != nil {
