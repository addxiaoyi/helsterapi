...
const maxCustomOAuthDiscoveryBytes int64 = 1 << 20
...
func FetchCustomOAuthDiscovery(c *gin.Context) {
	...
	if err := service.ValidateSSRFProtectedFetchURL(targetURL); err != nil {
		common.ApiErrorMsg(c, "Discovery URL 不允许访问: "+err.Error())
		return
	}
	...
	if resp.StatusCode != http.StatusOK {
		common.ApiErrorMsg(c, fmt.Sprintf("获取 Discovery 配置失败: 上游返回 %s", resp.Status))
		return
	}
	if resp.ContentLength > maxCustomOAuthDiscoveryBytes {
		common.ApiErrorMsg(c, "Discovery 配置超过大小限制")
		return
	}
	...
	if err = common.DecodeJson(bytes.NewReader(body), &discovery); err != nil {
