...
func ResponsesHelper(c *gin.Context, info *relaycommon.RelayInfo) (newAPIError *types.NewAPIError) {
	...
	if model_setting.GetGlobalSettings().PassThroughRequestEnabled || info.ChannelSetting.PassThroughBodyEnabled {
		...
		logger.LogDebug(c, "outbound request body bytes: %d", len(jsonData))
	...
	if resp != nil {
		...
		if !ok || httpResp == nil {
			return types.NewError(errors.New("adaptor returned invalid HTTP response"), types.ErrorCodeBadResponseBody, types.ErrOptionWithSkipRetry())
		}
	...
	if !ok || usageDto == nil {
		return types.NewError(errors.New("adaptor returned invalid usage"), types.ErrorCodeBadResponseBody, types.ErrOptionWithSkipRetry())
	}
	if info.RelayMode == relayconstant.RelayModeResponsesCompact {
