...
const (
	maxMidjourneyImageBytes     int64 = 25 << 20
	maxMidjourneyErrorBodyBytes int64 = 64 << 10
)
...
func RelayMidjourneyImage(c *gin.Context) {
	...
	if !model.IsAdmin(userID) && midjourneyTask.UserId != userID {
		c.JSON(http.StatusForbidden, gin.H{"error": "midjourney_task_access_denied"})
		return
	}
	...
	if resp.StatusCode != http.StatusOK {
		responseBody, _ := io.ReadAll(io.LimitReader(resp.Body, maxMidjourneyErrorBodyBytes))
		c.JSON(resp.StatusCode, gin.H{
			"error": string(responseBody),
		})
		return
	}
	if resp.ContentLength > maxMidjourneyImageBytes {
	...
	// 将图片流式传输到响应体
	_, err = io.Copy(c.Writer, io.LimitReader(resp.Body, maxMidjourneyImageBytes))
	if err != nil {
