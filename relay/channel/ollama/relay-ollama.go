...
func ollamaEmbeddingHandler(c *gin.Context, info *relaycommon.RelayInfo, resp *http.Response) (*dto.Usage, *types.NewAPIError) {
	...
	body, err := service.ReadResponseBodyLimited(resp.Body, 4<<20)
	if err != nil {
...
func PullOllamaModel(baseURL, apiKey, modelName string) error {
	...
	if response.StatusCode != http.StatusOK {
		return fmt.Errorf("拉取模型失败 %d", response.StatusCode)
	}
...
func DeleteOllamaModel(baseURL, apiKey, modelName string) error {
	...
	if response.StatusCode != http.StatusOK {
		return fmt.Errorf("删除模型失败 %d", response.StatusCode)
	}
...
func FetchOllamaVersion(baseURL, apiKey string) (string, error) {
	...
	body, err := service.ReadResponseBodyLimited(response.Body, 64<<10)
	if err != nil {
		return "", fmt.Errorf("读取响应失败: %v", err)
	}

	if response.StatusCode != http.StatusOK {
		return "", fmt.Errorf("查询版本失败 %d", response.StatusCode)
	}
