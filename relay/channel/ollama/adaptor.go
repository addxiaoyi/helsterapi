...
func (a *Adaptor) ConvertClaudeRequest(c *gin.Context, info *relaycommon.RelayInfo, request *dto.ClaudeRequest) (any, error) {
	...
	generalRequest, ok := openaiRequest.(*dto.GeneralOpenAIRequest)
	if !ok || generalRequest == nil {
		return nil, errors.New("Claude request conversion returned an invalid OpenAI request")
	}
	generalRequest.StreamOptions = &dto.StreamOptions{
		IncludeUsage: true,
	}
	// map to ollama chat request (Claude -> OpenAI -> Ollama chat)
	return openAIChatToOllamaChat(c, generalRequest)
}
