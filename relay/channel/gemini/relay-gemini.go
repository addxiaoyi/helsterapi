	...
	usage, err := geminiStreamHandler(c, info, resp, func(data string, geminiResponse *dto.GeminiChatResponse) bool {
		...
		if info.SendResponseCount == 0 {
			...
			if response.IsToolCall() {
				...
				if response.IsFinished() && len(response.Choices) > 0 {
...
func GeminiChatHandler(c *gin.Context, info *relaycommon.RelayInfo, resp *http.Response) (*dto.Usage, *types.NewAPIError) {
	responseBody, err := service.ReadResponseBodyLimited(resp.Body, 4<<20)
	if err != nil {
		return nil, types.NewOpenAIError(err, types.ErrorCodeBadResponseBody, http.StatusInternalServerError)
	}
	service.CloseResponseBodyGracefully(resp)
	logger.LogDebug(c, "Gemini response body bytes: %d", len(responseBody))
	var geminiResponse dto.GeminiChatResponse
...
func GeminiEmbeddingHandler(c *gin.Context, info *relaycommon.RelayInfo, resp *http.Response) (*dto.Usage, *types.NewAPIError) {
	...
	responseBody, readErr := service.ReadResponseBodyLimited(resp.Body, 4<<20)
	if readErr != nil {
...
func FetchGeminiModels(baseURL, apiKey, proxyURL string) ([]string, error) {
	...
	for page := 0; page < maxPages; page++ {
		...
		if response.StatusCode != http.StatusOK {
			_, _ = service.ReadResponseBodyLimited(response.Body, 64<<10)
			response.Body.Close()
			cancel()
			return nil, fmt.Errorf("服务器返回错误 %d", response.StatusCode)
		}

		body, err := service.ReadResponseBodyLimited(response.Body, 4<<20)
		response.Body.Close()
		cancel()
		if err != nil {
