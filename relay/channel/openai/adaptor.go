...
func (a *Adaptor) SetupRequestHeader(c *gin.Context, header *http.Header, info *relaycommon.RelayInfo) error {
	...
	if info.ChannelType == constant.ChannelTypeOpenRouter {
		...
		if header.Get("X-OpenRouter-Title") == "" {
			header.Set("X-OpenRouter-Title", "Helstare OS")
		}
	}
	return nil
}
...
func (a *Adaptor) ConvertImageRequest(c *gin.Context, info *relaycommon.RelayInfo, request dto.ImageRequest) (any, error) {
	...
	case relayconstant.RelayModeImagesEdits:
		if isJSONRequest(c) {
			// JSON image edits are accepted by several OpenAI-compatible clients,
			// but upstream edit endpoints commonly require multipart files.
			return convertJSONImageEditToMultipart(c, request)
		}
...
func convertJSONImageEditToMultipart(c *gin.Context, request dto.ImageRequest) (any, error) {
	var source string
	if len(request.Image) > 0 {
		if err := json.Unmarshal(request.Image, &source); err != nil {
			var imageObject struct {
				URL string `json:"url"`
			}
			if objectErr := json.Unmarshal(request.Image, &imageObject); objectErr != nil {
				return nil, fmt.Errorf("image must be a URL, data URL, or base64 string: %w", err)
			}
	...
	imageBytes, contentType, err := readImageSource(c, source)
	if err != nil {
		return nil, err
	}
	...
	part, err := writer.CreatePart(func() textproto.MIMEHeader {
		h := make(textproto.MIMEHeader)
...
func readImageSource(c *gin.Context, source string) ([]byte, string, error) {
	...
	if strings.HasPrefix(source, "data:") {
		...
		if len(decoded) > maxImageBytes {
			return nil, "", errors.New("image exceeds 20 MiB limit")
		}
		return decoded, strings.TrimPrefix(strings.SplitN(meta, ";", 2)[0], "data:"), nil
	}

	parsed, err := url.Parse(source)
	if err != nil || (parsed.Scheme != "http" && parsed.Scheme != "https") || parsed.Host == "" {
		return nil, "", errors.New("image must be an http(s) URL or base64 data URL")
	}
	...
	if resp.StatusCode < http.StatusOK || resp.StatusCode >= http.StatusMultipleChoices {
		return nil, "", fmt.Errorf("download image URL: upstream returned %s", resp.Status)
	}
