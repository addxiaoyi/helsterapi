...
func GeminiHelper(c *gin.Context, info *relaycommon.RelayInfo) (newAPIError *types.NewAPIError) {
	...
	if model_setting.GetGlobalSettings().PassThroughRequestEnabled || info.ChannelSetting.PassThroughBodyEnabled {
		...
		logger.LogDebug(c, "Gemini request body bytes: %d", len(jsonData))
	...
	if resp != nil {
		var responseOK bool
		httpResp, responseOK = resp.(*http.Response)
		if !responseOK || httpResp == nil {
			return types.NewError(fmt.Errorf("adaptor returned invalid HTTP response"), types.ErrorCodeBadResponseBody, types.ErrOptionWithSkipRetry())
		}
	...
	if httpResp == nil {
		return types.NewError(fmt.Errorf("adaptor returned empty HTTP response"), types.ErrorCodeBadResponse, types.ErrOptionWithSkipRetry())
	}
	...
	usageDto, usageOK := usage.(*dto.Usage)
	if !usageOK || usageDto == nil {
		return types.NewError(fmt.Errorf("adaptor returned invalid usage"), types.ErrorCodeBadResponseBody, types.ErrOptionWithSkipRetry())
	}
...
func GeminiEmbeddingHandler(c *gin.Context, info *relaycommon.RelayInfo) (newAPIError *types.NewAPIError) {
	...
	logger.LogDebug(c, "Gemini embedding request body bytes: %d", len(jsonData))
	body, size, closer, err := relaycommon.NewOutboundJSONBody(jsonData)
	if err != nil {
		return types.NewError(err, types.ErrorCodeConvertRequestFailed, types.ErrOptionWithSkipRetry())
	}
