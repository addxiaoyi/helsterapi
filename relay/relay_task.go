...
const maxTaskErrorResponseBytes int64 = 64 << 10
const maxTaskStatusResponseBytes int64 = 1 << 20
...
func RelayTaskSubmit(c *gin.Context, info *relaycommon.RelayInfo) (*TaskSubmitResult, *dto.TaskError) {
	...
	if resp != nil && resp.StatusCode != http.StatusOK {
		...
		_, _ = io.ReadAll(io.LimitReader(resp.Body, maxTaskErrorResponseBytes))
		return nil, service.TaskErrorWrapper(fmt.Errorf("upstream returned status %d", resp.StatusCode), "fail_to_fetch_task", resp.StatusCode)
	}
...
func tryRealtimeFetch(task *model.Task, isOpenAIVideoAPI bool) []byte {
	...
	body, err := io.ReadAll(io.LimitReader(resp.Body, maxTaskStatusResponseBytes+1))
	if err != nil {
		return nil
	}
	if int64(len(body)) > maxTaskStatusResponseBytes {
		return nil
	}
