...
const maxRerankResponseBytes int64 = 4 << 20

func RerankHandler(c *gin.Context, info *relaycommon.RelayInfo, resp *http.Response) (*dto.Usage, *types.NewAPIError) {
	responseBody, err := io.ReadAll(io.LimitReader(resp.Body, maxRerankResponseBytes+1))
	if err != nil {
		return nil, types.NewOpenAIError(err, types.ErrorCodeReadResponseBodyFailed, http.StatusInternalServerError)
	}
	service.CloseResponseBodyGracefully(resp)
	if int64(len(responseBody)) > maxRerankResponseBytes {
		return nil, types.NewOpenAIError(fmt.Errorf("rerank response exceeds %d bytes", maxRerankResponseBytes), types.ErrorCodeReadResponseBodyFailed, http.StatusBadGateway)
	}
	logger.LogDebug(c, "reranker response body bytes: %d", len(responseBody))
