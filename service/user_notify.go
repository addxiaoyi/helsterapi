...
func sendGotifyNotify(gotifyUrl string, gotifyToken string, priority int, data dto.Notify) error {
	...
	// Build the endpoint structurally so an existing base path/query cannot alter
	// the API path or produce a malformed token query.
	gotifyURL, err := url.Parse(gotifyUrl)
	if err != nil || gotifyURL.Scheme == "" || gotifyURL.Host == "" {
		return fmt.Errorf("invalid gotify URL")
	}
	if !strings.EqualFold(gotifyURL.Scheme, "http") && !strings.EqualFold(gotifyURL.Scheme, "https") {
		return fmt.Errorf("gotify URL must use http or https")
	}
	gotifyURL.Path = strings.TrimSuffix(gotifyURL.Path, "/") + "/message"
	query := gotifyURL.Query()
	query.Set("token", gotifyToken)
	gotifyURL.RawQuery = query.Encode()
	finalURL := gotifyURL.String()
	if err := ValidateSSRFProtectedFetchURL(finalURL); err != nil {
		return fmt.Errorf("request reject: %v", err)
	}
	...
	if system_setting.EnableWorker() {
		...
		req.Header.Set("User-Agent", "HelstareOS-Gotify-Notify/1.0")
