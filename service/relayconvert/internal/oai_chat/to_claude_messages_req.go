	...
	for _, message := range formatMessages {
		...
		if message.Role == "tool" {
			if len(claudeMessages) > 0 && claudeMessages[len(claudeMessages)-1].Role == "user" {
				...
				toolResults, ok := lastClaudeMessage.Content.([]dto.ClaudeMediaMessage)
				if !ok {
					toolResults = make([]dto.ClaudeMediaMessage, 0, 1)
				}
				lastClaudeMessage.Content = append(toolResults, dto.ClaudeMediaMessage{
					Type:      "tool_result",
