...
func ReadResponseBodyLimited(body io.Reader, maxBytes int64) ([]byte, error) {
	if maxBytes < 0 {
		return nil, fmt.Errorf("response body limit must not be negative")
	}

	response, err := io.ReadAll(io.LimitReader(body, maxBytes+1))
	if err != nil {
		return nil, err
	}
...
func IOCopyBytesGracefully(c *gin.Context, src *http.Response, data []byte) {
	...
	if src != nil {
		for k, v := range src.Header {
			if len(v) == 0 || !ShouldCopyUpstreamHeader(c, k, v) {
				continue
			}
