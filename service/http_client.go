...
func InitHttpClient() {
	transport := &http.Transport{
		...
		// Some OpenAI-compatible gateways reset HTTP/2 streaming bodies mid-response.
		// Relay traffic needs a stable HTTP/1.1 stream so the browser can receive the
		// complete SSE response instead of seeing a generic request failure.
		ForceAttemptHTTP2: false,
		// In Go 1.26, ForceAttemptHTTP2=false still allows automatic HTTP/2 when
		// the transport has no custom TLS or dialer. An empty map is the documented
		// explicit opt-out.
