...
const maxTaskPollingResponseBytes int64 = 1 << 20

func readTaskPollingBody(body io.Reader) ([]byte, error) {
	data, err := io.ReadAll(io.LimitReader(body, maxTaskPollingResponseBytes+1))
	if err != nil {
		return nil, err
	}
	if int64(len(data)) > maxTaskPollingResponseBytes {
		return nil, fmt.Errorf("task polling response exceeds %d bytes", maxTaskPollingResponseBytes)
	}
	return data, nil
}
...
func updateSunoTasks(ctx context.Context, channelId int, taskIds []string, taskM map[string]*model.Task) error {
	...
	responseBody, err := readTaskPollingBody(resp.Body)
	if err != nil {
	...
	if err != nil {
		logger.LogError(ctx, fmt.Sprintf("Get Suno Task parse body error2: %v, response_bytes: %d", err, len(responseBody)))
		return err
	}
...
func updateVideoSingleTask(ctx context.Context, adaptor TaskPollingAdaptor, ch *model.Channel, taskId string, taskM map[string]*model.Task) error {
	...
	// try parse as Helstare OS response format
	...
	if taskResult.Status == "" {
		...
		if err = common.Unmarshal(responseBody, &errorResult); err == nil {
			...
			if openaiError != nil {
				...
				logger.LogError(ctx, fmt.Sprintf("Task %s returned empty status with unrecognized error format, response_bytes: %d", taskId, len(responseBody)))
