...
func DecodeUrlImageData(imageUrl string) (image.Config, string, error) {
	...
	if err != nil {
		common.SysLog(fmt.Sprintf("fail to get image from url %s: %s", common.MaskSensitiveInfo(imageUrl), err.Error()))
	...
	for _, limit := range []int64{1024 * 8, 1024 * 24, 1024 * 64} {
		...
		limitReader := io.LimitReader(
			io.MultiReader(bytes.NewReader(readData), response.Body),
			int64(constant.MaxFileDownloadMB)*1024*1024+1,
		)
