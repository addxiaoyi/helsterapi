...
func (p *GenericOAuthProvider) ExchangeToken(ctx context.Context, code string, c *gin.Context) (*OAuthToken, error) {
	...
	if int64(len(body)) > maxGenericOAuthResponseBytes {
		return nil, fmt.Errorf("oauth token response exceeds %d bytes", maxGenericOAuthResponseBytes)
	}

	bodyStr := string(body)
	logger.LogDebug(ctx, "[OAuth-Generic-%s] ExchangeToken response body bytes: %d", p.config.Slug, len(bodyStr))
...
func (p *GenericOAuthProvider) GetUserInfo(ctx context.Context, token *OAuthToken) (*OAuthUser, error) {
	...
	if int64(len(body)) > maxGenericOAuthResponseBytes {
		return nil, fmt.Errorf("oauth userinfo response exceeds %d bytes", maxGenericOAuthResponseBytes)
	}

	bodyStr := string(body)
	logger.LogDebug(ctx, "[OAuth-Generic-%s] GetUserInfo response body bytes: %d", p.config.Slug, len(bodyStr))
